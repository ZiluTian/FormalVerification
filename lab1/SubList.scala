...
object SubList {
  ...
  def subListLength[T](l1: List[T], l2: List[T]): Unit = {
    ...
    (l1, l2) match {
        case (Cons(x, xs), Cons(y, ys)) if x == y && subList(xs, ys) => subListLength(xs, ys)
  ...
  def subListEqual[T](l1: List[T], l2: List[T]): Unit = {
    ...
    (l1, l2) match {
        case (Cons(x, xs), Cons(y, ys)) if x == y && subList(xs, ys) => subListEqual(xs, ys)
  ...
  def subListAntisym[T](l1: List[T], l2: List[T]): Unit = {
    ...
    (l1, l2) match {
        case (Cons(x, xs), Cons(y, ys)) => subListLength(l1, l2); subListLength(l2, l1); subListEqual(l1, l2)
        case _ => ()
    }
